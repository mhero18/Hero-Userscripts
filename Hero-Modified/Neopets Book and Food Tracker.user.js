...
const addBorder = (listId, element, pets) => {
    const alreadyHighlighted = element.classList.contains('saahphire-bft-to-do');
    ...
    if(pets && !alreadyHighlighted) element.title = `[${listId === 'gourmet' ? 'Feed' : 'Read'} to ${pets.join(', ')}] ` + element.title;
}
...
const orderSDB = images => {
    if(!sortItems.sdb) return;
    const moveHighlightedToTop = (items, highlightedItems) => {
        const uniqueHighlightedItems = highlightedItems
            .filter((item, index, allItems) => item && allItems.indexOf(item) === index);
        const parent = items[0]?.parentElement;
        if(!parent || !uniqueHighlightedItems.length) return;

        const unhighlightedItems = items.filter(item => !uniqueHighlightedItems.includes(item));
        const sortedItems = uniqueHighlightedItems.concat(unhighlightedItems);
        if(items.every((item, index) => item === sortedItems[index])) return;

        const highlightedFragment = document.createDocumentFragment();
        uniqueHighlightedItems.forEach(item => highlightedFragment.appendChild(item));
        const firstUnhighlightedItem = unhighlightedItems[0];
        if(firstUnhighlightedItem) parent.insertBefore(highlightedFragment, firstUnhighlightedItem);
        else parent.appendChild(highlightedFragment);
    }

    const rows = [...document.querySelectorAll('.sdb-item-cell')]
        .map(cell => cell.closest('tr'))
        .filter(row => row);
    const highlightedRows = images
        .map(image => image.closest('.sdb-item-cell')?.closest('tr'));
    moveHighlightedToTop(rows, highlightedRows);

    const gridItems = [...document.querySelectorAll('.sdb-grid-item')];
    const highlightedGridItems = images
        .map(image => image.closest('.sdb-grid-item'));
    moveHighlightedToTop(gridItems, highlightedGridItems);
}

const onSDB = () => {
    let refreshTimer;
    const refreshSDB = async () => {
        const highlighted = await highlightItems();
        orderSDB(highlighted);
    }
    const scheduleRefresh = () => {
        clearTimeout(refreshTimer);
        refreshTimer = setTimeout(refreshSDB, 80);
    }
