...
/*
*  Changes made by Hero:
*  - Added api call to itemDB to retrieve if Wearable for DTI Link
*  - makeApiCall flag to turn on/off
*  - DTI icon added for Trading Post and Auctions
*  - Added icons to Gallery
*  - Removed regular Shop Wizard icon
*  - Added sort_by=newest to TP Link
*
*
*/
...
let makeApiCall = false; //CHANGE AS NEEDED
...
if (isBeta) {
    ...
    */
    ...
    if (document.URL.includes("/island/tradingpost.phtml")) {
        if (makeApiCall) {
            const $items = $("img[src*='/items/']");
            const itemNames = [];

            $items.each((_, img) => {
                const name = img.nextSibling?.nodeValue?.trim();
            ...
            fetch("https://itemdb.com.br/api/v1/items/many", {
                ...
                .then(data => {
                // data is an object keyed by internal IDs, e.g. data["3580"]
                // Convert to array for easier search or build name -> item map
                const itemsByName = {};
                ...
                $items.each((_, img) => {
                    ...
                    let extras = { cash: false, wearable: false, tradeable: true, itemid: -1 };

                    if (itemData?.isWearable) {
                        extras.wearable = true;
                        extras.itemid = itemData.item_id || -1;
                    }

                    $(img.nextSibling).after(makelinks(name, extras));
                });
            })
                .catch(err => console.error("Error fetching batch item data:", err));
    ...
    if (document.URL.includes("auction_id")) {
        ...
        fixname = fixname.substr(0, fixname.indexOf(" (own"));
            ...
            if (itemNames.length > 0) {
                ...
                fetch("https://itemdb.com.br/api/v1/items/many", {
                    ...
                    $("a[href*='?type=bids&auction_id=']:not(:has('img'))").each(function () {
                        ...
                        $link.after(makelinks(name));

                        if (itemData && itemData.isWearable) {

                            // Build only the DTI icon link HTML manually:
                            let link = "https://impress.openneo.net/items?q=%s";
                            if (itemData.item_id !== -1 && itemData.item_id !== undefined) {
                                link = `https://impress.openneo.net/items/${itemData.item_id}`;
                            }
                            const dtiIconHtml = combiner(name, link, linkmap.dti.img);

                            // Append DTI icon inside the existing helper container:
                            $link.next(".search-helper").append(dtiIconHtml);
                        }
                    });

                })
                    .catch(err => console.error("Error fetching auctions item data:", err));
            }
        } else {
            // Auctions (original)
    ...
    if (document.URL.includes("gallery")) {
        $("img[src*='/items/']").each(function (k, v) {
            ...
            let itemname = nametd.text();
            itemname = itemname.replace(nametd.find(".medText").text(), "");
            nametd.find("b").after(makelinks(itemname));
        });
    }
