...
// @match        *://*.neopets.com/stamps.phtml?type=progress*
...
function progresssUpdated() {
    //Edd's code to make percentages readable
    let percentageCells = document.querySelectorAll('td:nth-child(4)');
    percentageCells.forEach(cell => {
        cell.style.backgroundColor = '#696969';
    });

    //Flutterz's code to add how many stamps there are
    let countCells = document.querySelectorAll('td:nth-child(3)');
    for (let i = 0; i < 100; i++){
        let stampCount = 0;
        if (STAMP_LIST[i] != undefined) {
            let currPage = STAMP_LIST[i];
            for (let j = 0; j < 25; j++){
                if (currPage.list[j].name!="No Stamp") stampCount++;
            }
        countCells[i].innerHTML = countCells[i].innerHTML + "<b>/" + stampCount + "</b>";
        }
    }
    return;
}
