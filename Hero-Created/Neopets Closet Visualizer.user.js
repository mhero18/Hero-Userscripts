// ==UserScript==
// @name        Neopets Closet Visualizer
// @version     1.0
// @author      Hero (special thanks to NeoQuest.Guide & itemDB)
// @icon        https://images.neopets.com/items/foo_gmc_herohotdog.gif
// @description Neopets Closet visualizer with a modern UI, dynamic filtering, and itemdb metadata.
...
const STORAGE_KEYS = {
  ...
  visitedPages: "visitedClosetPages",
  itemDatabase: "closetItemDatabase",
  scanMeta: "closetVisualizerScanMeta",
  helpCollapsed: "closetVisualizerHelpCollapsed",
  viewerTheme: "closetVisualizerViewerTheme",
  removalDraft: "closetVisualizerRemovalDraft",
};
...
const PAGE_SIZE = 30;
const VIEWER_REMOVAL_ENABLED = true;
...
console.log("NeopetsCloset Visualizer version:", scriptVersion);
...
function renderCollectorUI() {
  ...
  host.id = "closetVisualizerCollector";
  const isMainClosetPage = currentPageMeta.currentPage === 1;
  host.innerHTML = isMainClosetPage
    ? `
    <div class="sdbvc-panel">
      <div class="sdbvc-header">
        <div>
          <div class="sdbvc-title">Neopets Closet Visualizer Collector</div>
          <div class="sdbvc-subtitle">Use these buttons to collect your closet data and open the contextual report directly on Neopets.</div>
        </div>
      </div>
      <div class="sdbvc-help">
        ...
        <div id="sdbvcHelpContent">
          <ol class="sdbvc-helpList">
            ...
            <li>First time setup: click <strong>Scan Full Closet</strong>. This visits every closet page in the background and builds your full closet data.</li>
            ...
            <li>Later on, click <strong>Refresh Prices</strong> to update price data only, this won't need a full closet scan.</li>
      ...
      <div class="sdbvc-actions">
        <button type="button" id="sdbvcScanButton" class="sdbvc-btn scan">Scan Full Closet</button>
  ...
  style.textContent = `
    ...
    .sdbvc-viewerCardActionButton.is-placeholder {
      visibility: hidden;
      pointer-events: none;
    }
    .sdbvc-viewerImage,
  ...
  buttons.scan.addEventListener("click", () => {
    runTask(buttons.scan, async () => {
      setStatus("Starting automatic full closet scan...");
      const result = await scanEntireCloset({
        onProgress(progress) {
          const attemptLabel = progress.attempt ? ` (attempt ${progress.attempt}/${progress.attempts})` : "";
          setStatus(`Scanning closet page ${progress.pageIndex}/${progress.totalPages}${attemptLabel}...`);
          showOverlay({
            title: "Scanning Closet",
            text: `Fetching closet page ${progress.pageIndex} of ${progress.totalPages}${attemptLabel} in the background. Please wait until the scan finishes.`,
      ...
      if (result.failedPages.length) {
        ...
        setStatus(
          `Full closet scan finished with ${result.failedPages.length} failed page(s). Saved all successful pages. Please manually visit page(s): ${listedPages}${moreLabel}.`,
        );
      } else {
        setStatus("Full closet scan complete.");
      }
    }).catch((error) => {
      hideOverlay();
      setStatus(`Full closet scan failed: ${error.message}`);
    });
  });
  ...
  buttons.exportFull.addEventListener("click", () => {
    ...
    if (!payload.items.length) {
      setStatus("No stored closet data yet. Run a scan first.");
      return;
    }
    const result = exportVisualizerPayload(payload, `closet-visualizer-${snapshotMode}`);
    setStatus(result.message);
  });

  buttons.clearZones.addEventListener("click", () => {
    if (!confirm("Clear only the saved wearable zoning cache? This will keep your closet scan data and itemdb data, but wearable zone lookups will need to be fetched again.")) {
      return;
    }
    clearZoningCache();
...
function getCurrentPageMeta(doc) {
  const hiddenPage = parseInt(doc.querySelector("input[name='page']")?.getAttribute("value") || "", 10);
  const currentUrl = safeParseUrl(doc.URL || doc.baseURI || location.href, location.href);
  const currentPage = Number.isFinite(hiddenPage) && hiddenPage > 0
    ? hiddenPage
    : parseInt(currentUrl?.searchParams.get("page") || "1", 10) || 1;
  const linkedPages = getClosetLinkedPages(doc);
  const totalPages = Math.max(currentPage, ...linkedPages, 1);
  const totalsText = doc.body?.textContent?.replace(/,/g, "") || "";
  const totalsMatch = totalsText.match(/Items:\s*(\d+)\s*\|\s*Qty:\s*(\d+)/i);
  const username =
...
function getClosetLinkedPages(doc) {
  const pageNumbers = new Set();
  doc.querySelectorAll("a[href*='closet.phtml']").forEach((link) => {
    const parsed = safeParseUrl(link.getAttribute("href"), location.href);
...
function parseClosetRows(doc) {
  const table = findClosetItemsTable(doc);
  if (!table) {
    return [];
  }
  const pageNumber = getCurrentPageMeta(doc).currentPage;

  return Array.from(table.querySelectorAll("tr"))
    .filter((row) => row.querySelector("input.delete_pb, input[type='text'][name]"))
    .filter((row) => row.querySelectorAll("td").length >= 6)
    .map((row) => {
      const cells = row.querySelectorAll("td");
      const deleteButton = row.querySelector("input.delete_pb");
      const quantityInput = row.querySelector("input[type='text'][name]");
      const itemId = Number(
        deleteButton?.getAttribute("data-item_id") ||
        quantityInput?.getAttribute("name") ||
        0,
      );
      const name = getClosetItemName(cells[1]);
      const qty = parseInt(cells[4]?.textContent.replace(/[^\d]/g, "") || "0", 10);
      ...
      return {
        ...
        closetType: cells[3]?.textContent.trim() || "",
        pageNumber,
        removalMode: deleteButton ? "delete-button" : "quantity-input",
        isTradeable: !deleteButton,
        isRemovable: !deleteButton,
      };
    })
    .filter((item) => Number.isFinite(item.id) && item.id > 0);
}

function getClosetItemName(cell) {
  if (!cell) return "";
  const beforeBreak = extractTextBeforeFirstBreak(cell).replace(/\s+/g, " ").trim();
  return beforeBreak || cell.textContent?.trim() || "";
}

function extractTextBeforeFirstBreak(node) {
  if (!node) return "";
  ...
  for (const child of Array.from(node.childNodes)) {
    ...
    text += extractTextBeforeFirstBreak(child);
  }
  return text;
}

function findClosetItemsTable(doc) {
  const selectors = [
    "form[action*='process_closet.phtml'] table",
    ".content form[action*='process_closet.phtml'] table",
    ".content table",
    "table",
  ];
  const candidateTables = selectors.flatMap((selector) => Array.from(doc.querySelectorAll(selector)));
  return candidateTables.find((table) => {
    if (table.querySelector("input.delete_pb, input[type='text'][name]")) {
      return true;
    }
    const headerText = Array.from(table.querySelectorAll("th")).map((th) => th.textContent?.trim() || "").join(" | ");
    return /Image/.test(headerText) && /Name/.test(headerText) && /Remove\?/.test(headerText);
  }) || null;
}
...
function recordCurrentPage(doc, pageNumber) {
  const items = parseClosetRows(doc);
...
async function scanEntireCloset({ onProgress } = {}) {
  const pageMeta = getCurrentPageMeta(document);
  const totalPages = await determineClosetPageCount(pageMeta.totalPages || 1);
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    const scanResult = await fetchAndParseClosetPage({
      pageNumber,
      pageIndex: index + 1,
    ...
    if (scanResult.valid) {
      pageItemIdsByPage[String(pageNumber)] = scanResult.items.map((item) => item.id);
    }
    if (!scanResult.valid) {
      failedPages.push({
  ...
  setVisitedPages(pages.filter((pageNumber) => !failedPageNumbers.has(pageNumber)));
  setScanMeta({
    ...getScanMeta(),
    totalPages: pages.length,
    totalItems: uniqueItems.length,
    totalQuantity: uniqueItems.reduce((sum, item) => sum + Number(item.qty || 0), 0),
...
async function determineClosetPageCount(initialPageCount) {
  let totalPages = Math.max(1, Number(initialPageCount) || 1);
  let previousTotalPages = 0;

  while (totalPages !== previousTotalPages) {
    previousTotalPages = totalPages;
    const pagesToInspect = Array.from({ length: totalPages }, (_, index) => index + 1)
      .filter((pageNumber) => pageNumber === 1 || pageNumber === totalPages);

    for (const pageNumber of pagesToInspect) {
      const responseText = await fetchClosetPage(buildClosetPageUrl(pageNumber));
      const doc = new DOMParser().parseFromString(responseText, "text/html");
      const pageMeta = getCurrentPageMeta(doc);
      totalPages = Math.max(totalPages, pageMeta.totalPages || 1);
    }
  }
...
async function fetchAndParseClosetPage({ pageNumber, pageIndex, totalPages, onProgress }) {
  const url = buildClosetPageUrl(pageNumber);
  ...
  for (let attempt = 1; attempt <= PAGE_SCAN_RETRY_LIMIT; attempt += 1) {
    ...
    const responseText = await fetchClosetPage(url);
    const doc = new DOMParser().parseFromString(responseText, "text/html");
    const validation = validateClosetPage(doc, pageNumber);
    if (validation.valid) {
      return {
        valid: true,
...
function validateClosetPage(doc, pageNumber) {
  const pageMeta = getCurrentPageMeta(doc);
  const items = parseClosetRows(doc);
  const expectedPage = parseInt(doc.querySelector("input[name='page']")?.getAttribute("value") || String(pageMeta.currentPage || pageNumber), 10);

  if (expectedPage !== pageNumber) {
    return { valid: false, items, reason: `Expected page ${pageNumber} but found ${expectedPage}` };
  }
  if (!findClosetItemsTable(doc)) {
    return { valid: false, items, reason: "Missing closet items table" };
  }
  if (items.length > PAGE_SIZE) {
    return { valid: false, items, reason: `Expected at most ${PAGE_SIZE} rows but found ${items.length}` };
  }

  return { valid: true, items };
}

function buildClosetPageUrl(pageNumber) {
  if (pageNumber <= 1) {
    return `${location.origin}/closet.phtml`;
  }
  return `${location.origin}/closet.phtml?obj_name=&page=${pageNumber}&category=0&per_page=${PAGE_SIZE}`;
}

async function fetchClosetPage(url) {
...
function safeParseUrl(rawUrl, baseUrl) {
  try {
...
function buildVisualizerExportPayload({ snapshotMode = "partial", itemScope = "snapshot" } = {}) {
  ...
  return {
    source: "nqg-closet-visualizer-collector",
...
async function updateItemdbData({ mode = "itemdb", onProgress } = {}) {
  const items = getStoredItems();
  if (!items.length) {
    throw new Error("No closet items have been collected yet. Run a scan first.");
  }
...
function openEmbeddedVisualizerReport({ status }) {
  ...
  const payload = buildVisualizerExportPayload({ snapshotMode, itemScope: "stored" });
  if (!payload.items.length) {
    status("No stored closet data yet. Run a scan first.");
    return;
  }
    ...
    <div class="sdbvc-viewerShell">
      <div class="sdbvc-viewerTopbar">
        <div class="sdbvc-viewerTopbarIntro">
          <strong>Neopets Closet Visualizer</strong>
      ...
      <div class="sdbvc-viewerBody">
        <section class="sdbvc-viewerHero">
          <div class="sdbvc-viewerHeroCopy">
            <div class="sdbvc-viewerEyebrow">Neopets Closet</div>
            <h1 class="sdbvc-viewerTitle" data-role="title">Neopets Closet Visualizer</h1>
            <p class="sdbvc-viewerHeroText">Explore your full closet in one place with richer filters, images, and itemDB metadata.</p>
          </div>
        ...
        <div class="sdbvc-viewerGrid">
          <div class="sdbvc-viewerLeft">
            ...
            <section class="sdbvc-viewerPanel">
              <div class="sdbvc-viewerHeading">
                <div class="sdbvc-viewerHeadingMain">
                  <h2>Filters</h2>
                  <p>Mix native closet fields with itemDB metadata.</p>
                </div>
              ...
              <div class="sdbvc-viewerFilters">
                ...
                <label><span>Closet Type</span><select data-filter="closetType"></select></label>
          ...
          <aside class="sdbvc-viewerAside">
            ...
            ${VIEWER_REMOVAL_ENABLED ? `<section class="sdbvc-viewerSubPanel" data-role="removalPanel">
              <div class="sdbvc-viewerSubPanelHeader">
                <div>
                  <h3>Removal List</h3>
                  <p>Queue removable closet items here, then test closet submission directly from the visualizer.</p>
                </div>
              </div>
...
function normalizeViewerItem(item) {
  return {
    ...
    image: item.image || "",
    description: item.description || "",
    closetType: item.closetType || item.type || item.cat || "",
    pageNumber: Number(item.pageNumber || 0) || null,
...
function populateViewerFilterOptions() {
  ...
  fillViewerSelect(
    visualizerOverlayElements.host.querySelector('[data-filter="closetType"]'),
    "All Closet Types",
    [...new Set(items.map((item) => item.closetType).filter(Boolean))].sort(),
    viewerState.filters.closetType,
  );
  fillViewerSelect(
...
function renderContextualViewerOverview() {
  ...
  visualizerOverlayElements.title.textContent = dataset.username ? `Closet Visualizer - ${dataset.username}` : "Closet Visualizer";
...
function renderContextualViewerResults() {
  ...
  const uniqueTypes = new Set(visibleItems.map((item) => item.closetType).filter(Boolean));
  ...
  visualizerOverlayElements.chips.innerHTML = [
    ["Visible Closet Types", uniqueTypes.size],
  ...
  visualizerOverlayElements.detail.innerHTML = `
    <div class="sdbvc-viewerDetailHero">
      ...
      <div class="sdbvc-viewerDetailBody">
        ...
        <p class="sdbvc-viewerDetailMeta">${escapeHtml(item.closetType || "Unknown Type")} | ID ${escapeHtml(String(item.id))}</p>
        <div class="sdbvc-viewerChips">
          ...
          ${!item.isTradeable ? `<span class="sdbvc-viewerPill">Not Tradeable</span>` : ""}
          ${!item.isRemovable ? `<span class="sdbvc-viewerPill">Not Removable</span>` : ""}
        </div>
      </div>
    </div>
  ...
  visualizerOverlayElements.removalList.innerHTML = `
    <div class="sdbvc-viewerRemovalList">
      ${entries.length
        ...
        `).join("")}</div>`
        : `<div class="sdbvc-viewerRemovalEmpty">Add removable closet items here. Items with a Delete item button cannot be queued.</div>`}
      <div class="sdbvc-viewerRemovalControls">
        <div class="sdbvc-viewerRemovalPrimaryRow">
          <label class="sdbvc-viewerRemovalPinRow">
            <span>Closet PIN</span>
...
function addViewerItemToRemovalList(itemId) {
  ...
  if (!item) return;
  if (!item.isRemovable) {
    setViewerRemovalStatus(`${item.name} cannot be removed from the closet with this workflow.`, true);
    renderContextualViewerRemovalList();
    return;
  }
...
function addFilteredItemsToRemovalList() {
  ...
  const itemsToAdd = filteredItems
    .filter((item) => item.isRemovable && Number(item.qty || 0) > 0 && !queuedIds.has(item.id))
...
function getViewerRemovalEntries() {
  ...
  return Object.entries(viewerState.removalList)
    .map(([itemId, qty]) => {
      ...
      if (!item || !item.isRemovable) return null;
      return {
        id: item.id,
        name: item.name,
...
async function submitViewerRemovalList() {
  if (!VIEWER_REMOVAL_ENABLED) {
    setViewerRemovalStatus("Closet removal is enabled for testing in this visualizer.", false);
    renderContextualViewerRemovalList();
    return;
  }
  ...
  try {
    ...
    const result = await submitClosetRemovalRequest(entries, viewerState.removalPin);
    if (result.ok) {
    ...
    setViewerRemovalStatus(error?.message || "Closet removal request failed.", true);
  } finally {
...
async function submitClosetRemovalRequest(entries, pin) {
  const form = new URLSearchParams();
  form.append("page", "1");
  entries.forEach((entry) => {
    form.append(String(entry.id), String(Math.max(0, Number(entry.qty) || 0)));
  });
  form.append("pin", String(pin || "0"));

  const response = await fetch("/process_closet.phtml", {
    method: "POST",
    credentials: "include",
    headers: {
  ...
  if (!response.ok) {
    return { ok: false, message: `Closet removal request failed with ${response.status}.` };
  }

  const parsedMessage = parseClosetRemovalResponseMessage(html);
  ...
  return {
    ok: true,
    message: parsedMessage.message || `Removed ${formatNumber(totalRequested)} item${totalRequested === 1 ? "" : "s"} from your closet.`,
  };
}

function parseClosetRemovalResponseMessage(html) {
...
function getViewerFilteredItems() {
  ...
  return items
    .filter((item) => {
      if (search) {
        const haystack = [item.name, item.description, item.closetType, item.itemdb?.category, item.itemdb?.description]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
        if (!haystack.includes(search)) return false;
      }
      if (viewerState.filters.closetType !== "all" && item.closetType !== viewerState.filters.closetType) return false;
...
function buildViewerCardMarkup(item) {
  const showCategoryBubble = item.closetType?.toLowerCase() === "neocash" && Boolean(item.itemdb?.category);
  return `
    <article class="sdbvc-viewerCard" data-item-id="${item.id}">
      ...
      <div class="sdbvc-viewerCardActionSlot">
        ${VIEWER_REMOVAL_ENABLED && item.isRemovable
          ...
          : `<span class="sdbvc-viewerCardActionButton is-placeholder" aria-hidden="true">Add to Removal</span>`}
      </div>
      <div class="sdbvc-viewerChips">
        <span class="sdbvc-viewerPill">${escapeHtml(item.closetType || "Unknown Type")}</span>
            ...
            .map(
              (item) => `
                <tr data-item-id="${item.id}">
                  ...
                  <td>${escapeHtml(item.closetType || "")}</td>
...
function buildViewerDetailLinks(item) {
  ...
  if (VIEWER_REMOVAL_ENABLED && item.isRemovable) {
    links.unshift({
  ...
  if (item.isTradeable && !isViewerNcItem(item)) {
    links.push(
      {
        label: "Auction House",
